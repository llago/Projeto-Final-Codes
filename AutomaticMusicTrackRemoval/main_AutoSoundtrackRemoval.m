...
Fs = 48000;
mixtureStereo = resample(mixtureStereo,Fs,Fs1);
musicTrackStereo = resample(musicTrackStereo,Fs,Fs2);
...
%musicTrack = musicTrack./max(abs(musicTrack));
    ...
    last_non_zero = size(musicTrack,1)-find(musicTrack(end:-1:1) ~= 0)+2;
    musicTrack2 = musicTrack(1:last_non_zero);
